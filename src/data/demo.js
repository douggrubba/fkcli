...
async function runDemo() {
    ...
    standings.forEach((team, index) => {
        ...
        console.log(
            `${index + 1}. ${team.city} ${team.name}: ${team.wins}-${team.losses} (${winPct}%)`
        );
    });
    ...
    standings.slice(0, 3).forEach((team) => {
        ...
        if (profile && profile.stadium) {
            ...
            console.log(
                `  Stadium: ${profile.stadium.name} (${profile.stadium.capacity.toLocaleString()} capacity)`
            );
    ...
    standings.slice(0, 3).forEach((team) => {
        ...
        if (profile && profile.history) {
            ...
            console.log(
                `${team.city} ${team.name}: ${totalChampionships} championships (latest: ${latestChampionship})`
            );
        }
    });
    ...
    console.log(
        `Updated Red Sox record: ${updatedRecord.wins}-${updatedRecord.losses} (${(updatedRecord.win_percentage * 100).toFixed(1)}%)`
    );
    ...
    console.log(
        `Updated Red Sox 2025 attendance: ${updatedProfile.fanbase.attendance2025.toLocaleString()}`
    );
